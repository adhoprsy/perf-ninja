      ...
      for (int j = 0; j < N; j++) {
        ...
        for (int p = -1; p <= 1; p++) {   // row-offet (-1,0,1)
          for (int q = -1; q <= 1; q++) { // col-offset (-1,0,1)
            if (__builtin_expect(
                    (i + p < 0) || // if row offset less than UPPER boundary
                        (i + p >
                         M - 1) || // if row offset more than LOWER boundary
                        (j + q <
                         0) || // if column offset less than LEFT boundary
                        (j + q > N - 1),
                    1)) // if column offset more than RIGHT boundary
              continue;
...
#ifdef SOLUTION
        int next[10] = {0, 0, current[i][j], 1, 0, 0, 0, 0, 0, 0};
        future[i][j] = next[aliveNeighbours];
#else
